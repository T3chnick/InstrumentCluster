void UpdDisplays() {
  ...
  if (millis() - cDispTimer > c_Disp_Interval) { UpdKM(); cDispUpd(); cDispTimer = millis(); }
...
void cDispUpd() {
  ...
  switch (Disp_Page) {
    case 0: { Sub_Page_Max = 4; Vol_Butt_Busy = 0;  
        switch (Disp_Sub_Page) {
          case 0: { c_Disp_Interval = 2000;  // odometr
              ...
              c.setCursor(35, 56);
              c.print(odometr);
              if (buttR.isHold()) { Trip_KM_Reset = Odometr_Pulses; Trip_KM_Time = 0; SaveKm(); }
            } break;
    ...
    case 1: { Sub_Page_Max = 2; Vol_Butt_Busy = 0; c_Disp_Interval = 1000; 
        ...
        switch (Disp_Sub_Page) {  
          case 0: {  // AFTER START
            ...
            float FuelCons = (( (af_Start_KM_Inj_Milis * (Inj_Flow/60.0/1000.0)) /1000.0 ) / Trip_KM) * 100;
            c.setCursor(26, 52);
          ...
          case 1: {  // AFTER RESET
            ...
            float FuelCons = (( (af_Reset_Inj_Milis * (Inj_Flow/60.0/1000.0)) /1000.0 ) / Trip_KM) * 100;
            c.setCursor(26, 52);
            (FuelCons > 0 && FuelCons < 40) ? c.print(FuelCons,1) : c.print("--.-");
            c.print(" L.100KM");
            }break;
          ...
          case 2: { c_Disp_Interval = 400; // ECO
            PrintMsgF(53, 11, "ECO"); 
              ...
              float FuelNow;
              if(sp) { FuelNow = ((Inj_Flow/60.0/1000.0) * injTime / 1000.0) *th *30 /sp *100;
                PrintMsgF(41, 52, "L/100KM"); 
              }
                else { FuelNow = ((Inj_Flow/60.0/1000.0) * injTime / 1000.0) *th *30;
                  PrintMsgF(54, 52, "L/H");
                }
    ...
    case 2: { Sub_Page_Max = 8; Vol_Butt_Busy = 1; c_Disp_Interval = 60000;  //Settings
        ...
        switch (Disp_Sub_Page) {       
          ...
          case 1: {  // set hour
              if(v_Key_Event) { 
                if(v_Key_Event == 1){ H++; if (H > 23) { H = 0; }} 
                else if (v_Key_Event == 2) { H--; if (H < 0) { H = 23; }}
                setRTC.setHour(H); 
              }
              PrintClock(H, M);
          ...
          case 2: {  // set minute
              if(v_Key_Event) {
                if(v_Key_Event == 1) { M++; if (M > 59) { M = 0; }}
                else if (v_Key_Event == 2) { M--; if (M < 0) { M = 59; }}
          ...
          case 5: {
              if(v_Key_Event) { (v_Key_Event == 1) ? CC_Kp += 0.1 : CC_Kp -= 0.1; 
                saveCCpid();
              }            
              PrintCC();
          ...
          case 6: {
              if(v_Key_Event) { (v_Key_Event == 1) ? CC_Ki += 0.02 : CC_Ki += 0.02; 
                saveCCpid();
              }                       
              PrintCC();
          ...
          case 7: {
              if(v_Key_Event) { (v_Key_Event == 1) ? CC_Kd += 0.02 : CC_Kd += 0.02;  
                saveCCpid();
              }              
              PrintCC();
          ...
          case 8: {  // Inj_Flow
              if(v_Key_Event) { (v_Key_Event == 1) ? Inj_Flow += 1000 : Inj_Flow -= 1000; 
                SaveIntervals();
              }
              PrintInj_Flow();
              c.drawRect(9, 24, 104, 15, WHITE);
...
void PrintServiceSet() {
  ...
  PrintMsgF(13, 28, "Service_A_KM"); PrintMsgN(82, 28, Service_A_interval);
  PrintMsgF(13, 44, "Service_B_KM"); PrintMsgN(82, 44, Service_B_interval);
  }
...
void PrintInj_Flow() {
  PrintMsgF(41, 11, "INJ SET");
  
  PrintMsgF(12, 28,  "INJ_FLOW");
  PrintMsgFl(92, 28, Inj_Flow, 0);

  float liter = ( (af_Reset_Inj_Milis * (Inj_Flow/60.0/1000.0)) /1000.0 );
  PrintMsgF(11, 46,  "TOTAL.CONS");
  PrintMsgFl(82, 46, liter, 1);
  }
